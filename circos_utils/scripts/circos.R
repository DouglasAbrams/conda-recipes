...
plot_remixt = function(copy_number){
  ...
  #print(copy_number[copy_number$major_raw > 8, ]$major_raw)
  #copy_number[copy_number$major > 8, ]$major = 9
  #copy_number[copy_number$minor > 8, ]$minor = 9
  chrom_maxes = rep(-1, length(unique(remixt$chromosome)))
  names(chrom_maxes) = unique(remixt$chromosome)
  for (chrom in names(chrom_maxes)){
    chrom_maxes[chrom] = max(remixt[remixt$chromosome==chrom, ]$start)
  }
  ...
  circos.track(factors= paste0("chr", copy_number$chromosome),
  ...
  circos.track(factors= paste0("chr", cn_combined$chromosome),
               ylim=c(0, 8), "track.height" = 0.15)
...
plot_copy_number = function(copy_number){
...
circos_plot = function(titan, remixt, svs, label){
  require(circlize)
...
make_circos = function(titan, remixt, svs, outfile, sample_label){

  #save plot to pdf
  pdf(outfile, 8, 9)

  #create plot
  circos_plot(titan, remixt, svs, sample_label)
